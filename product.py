...
class productClass():
    def __init__(self,root):
        ...
        #title
        title=Label(product_Frame,text=" ManageProduct Details",font=("goudy old style",18),bg="#0f4d7d",fg="white").pack(side=TOP,fill=X)
        ...
        cmb_status.current(0)
         ...
         # button
        btn_add=Button(product_Frame,text="Save",font=("gpudy old style",15),bg="#2196f3",fg="white",cursor="hand2").place(x=10,y=400,width=100,height=40)
        btn_update=Button(product_Frame,text="Upadte",font=("gpudy old style",15),bg="#4caf50",fg="white",cursor="hand2").place(x=120,y=400,width=100,height=40)
        btn_delete=Button(product_Frame,text="Delete",font=("gpudy old style",15),bg="#f44336",fg="white",cursor="hand2").place(x=230,y=400,width=100,height=40)
        btn_clear=Button(product_Frame,text="Clear",font=("gpudy old style",15),bg="#607d8b",fg="white",cursor="hand2").place(x=340,y=400,width=100,height=40)
    ...
    def add(self):
        ...
        try:
            if self.var_cat.get()=="Select" or self.var_cat.get()=="Empty"or self.var_sup.get()=="Select" or self.var_name.get()=="":
                ...
                row=cur.fetchnone()
                if row!=None:
                    messagebox.showererror("Error","Product already present, try different",parent=self.root)
                else:
    ...
    def update(self):
        ...
        try:
            if self.var_pid.get()=="":
                ...
                row=cur.fetchnone()
                if row==None:
                    messagebox.showererror("Error","Invalid product ",parent=self.root)
                else:
    ...
    def search(self):
      ...
      txt_search=Entry(SearchFrame,textvariable=self.var_searchtxt, font=("gpudy old style",15),bg="lightyellow").place(x=200,y=10)
      btn_search=Button(SearchFrame,text="Search",font=("gpudy old style",15),bg="#4caf50",fg="white",cursor="hand2").place(x=430,y=8,width=150,height=30)
      ...
      Scrollbar(command=self.product_table.xview) 
      Scrollbar(command=self.product_table.yview) 
