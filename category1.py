...
class categoryClass():
    def __init__(self,root):
        ...
        # ========title========
        lbl_title=Label(self.root,text="Manage Product Category",font=("goudy old style",30),bg="#184a45",fg="white",bd=3,relief=RIDGE ).pack(side=TOP, fill=X, padx=10,pady=29)
        ...
        btn_add=Button(self.root,text="ADD",command=self.add,font=("goudy old style",18),bg="#4caf50", fg="white",cursor="hand2").place(x=360, y=170, width=300)
        btn_delete=Button(self.root,text="DELETE",command= self.delete,font=("goudy old style",18),bg="#4caf50", fg="white",cursor="hand2").place(x=520, y=170, width=300)
        ...
        cat_frame.place(x=700,y=350,width=380,height=150)
        ...
        Scrollbar(command=self.category_table.xview) 
        Scrollbar(command=self.category_table.yview) 
    ...
    def add(self):
        ...
        try:
            if self.var_name.get()=="":
               ...
               cur.execute("Select * from catgory where name=?",(self.var_namee.get(),))
               row=cur.fetchnone()
            if row!=None:
                messagebox.showererror("Error","category already present, try different",parent=self.root)
            else:
    ...
    def delete(self):
        ...
        try:
            if self.var_cat_id.get()=="":
                ...
                cur.execute("Select * from category where cid=?",(self.var_cat_id(),))
                row=cur.fetchone()
                if row==None:
                    ...
                    if op==True:
                       cur.execute("delete from category table where cid=?",(self.var_cat_id.get(),))
                       con.commit()
