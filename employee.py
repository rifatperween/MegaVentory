...
class employeeClass():
    def __init__(self,root):
        ...
        btn_search=Button(SearchFrame,text="Search",command=self.search,font=("gpudy old style",15),bg="#4caf50",fg="white",cursor="hand2").place(x=430,y=8,width=150,height=30)
        ...
        # row 3
        lbl_email=Label(self.root,text="Emial",font=("goudy old style",15),bg="white").place(x=50,y=230)
        ...
        btn_update=Button(self.root,text="Upadte",command=self.update,font=("goudy old style",15),bg="#4caf50",fg="white",cursor="hand2").place(x=620,y=305,width=110,height=28)
        ...
        Scrollbar(command=self.EmployeeTable.xview) 
        Scrollbar(command=self.EmployeeTable.yview) 
    ...
    def add(self):
        ...
        try:
            if self.var_emp_id.get()=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showererror("Error","This Employee ID already assigned, try different",parent=self.root)
                else:
    ...
    def update(self):
        ...
        try:
            if self.var_emp_id.get()=="":
                ...
                row=cur.fetchone()
                if row==None:
                    messagebox.showererror("Error","Invalid Employee ID",parent=self.root)
                else:
